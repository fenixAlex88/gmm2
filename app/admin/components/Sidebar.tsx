...
import { Database, Plus, Trash2, X, Heart, Search } from 'lucide-react';
...
export default function Sidebar({ articles, metadata, onSelect, selectedId, onRefresh }: any) {
	...
	// Выдаленне метададзеных (тэгі, аўтары і г.д.)
	...
	return (
		<aside className="w-80 flex flex-col bg-white border-r border-slate-200 shadow-xl z-10 h-full">
			{/* ВЕРХНЯЯ ЧАСТКА: Загаловак, Пошук і Табы */}
			<div className="p-4 border-b bg-slate-50/50 space-y-4">
				...
				<div className="relative group">
					...
					{searchQuery && (
						<button
							...
							className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 bg-slate-100 rounded-md transition-colors"
						>
							<X size={14} />
						</button>
					)}
				...
				{/* ПЕРАКЛЮЧАЛЬНІК УКЛАДАК */}
				<div className="flex p-1 bg-slate-200/50 rounded-xl">
					<button
						onClick={() => setActiveTab('articles')}
						className={`flex-1 py-2 text-[10px] font-black rounded-lg transition-all ${activeTab === 'articles' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500 hover:text-slate-700'}`}
					>АРТЫКУЛЫ</button>
					<button
						onClick={() => setActiveTab('database')}
						className={`flex-1 py-2 text-[10px] font-black rounded-lg transition-all ${activeTab === 'database' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500 hover:text-slate-700'}`}
					>БАЗА</button>
				</div>
			</div>

			{/* ОСНЎНЫ КАНТЭНТ (СПІСЫ) */}
			<div className="flex-grow overflow-y-auto custom-scrollbar">
				{activeTab === 'articles' ? (
						...
						{filteredArticles.length > 0 ? (
							filteredArticles.map((a: any) => (
								<div
									key={a.id}
									onClick={() => onSelect(a)}
									className={`group p-4 border-b cursor-pointer transition-all hover:bg-amber-50/30 ${selectedId === a.id ? 'bg-amber-50 border-l-4 border-l-amber-600' : 'border-l-4 border-l-transparent'}`}
								>
									<div className="flex justify-between items-start gap-2">
										<h3 className="text-sm font-bold text-slate-700 line-clamp-2 leading-tight group-hover:text-slate-900 transition-colors">
											{a.title}
										</h3>
										<button
									...
									<div className="flex gap-2 mt-2">
										<span className="text-[9px] px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full font-bold uppercase tracking-wider">
					...
					<div className="p-4 space-y-8">
								...
								<div className="flex flex-wrap gap-2">
									{items.length > 0 ? (
										items.map((item: any) => (
											<div
												key={item.id}
												className="flex items-center gap-1.5 px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 hover:border-amber-300 transition-colors shadow-sm"
											>
												{item.name}
												<button
													onClick={() => handleDeleteMetadata(key.slice(0, -1), item.id)}
						...
						{/* НІЖНЯЯ ПАНЭЛЬ АЧЫСТКІ */}
						<div className="pt-6 border-t border-slate-100 space-y-3">
							<button
								onClick={handleFullCleanup}
								className="w-full py-3 px-4 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg"
							>
								<Trash2 size={14} /> Ачысціць файлы на серверы
							</button>
							<p className="text-[9px] text-slate-400 text-center leading-relaxed px-4">
								Гэта праскануе базу і выдаліць з папак <code className="text-amber-600 font-bold">uploads</code> і <code className="text-amber-600 font-bold">storage</code> файлы, якія не выкарыстоўваюцца.
							</p>
						</div>
					</div>
				)}
